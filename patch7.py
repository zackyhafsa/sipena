...
html = html.replace('<form wire:submit.prevent="submit" class="space-y-8">', '<form wire:submit.prevent="submit">\n        <div class="flex flex-col lg:flex-row gap-6">\n            <div class="lg:w-3/4 w-full">')

# Modify question dive
old_div = r'<div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 scroll-mt-28"\s*id="question-\{\{ \$index \}\}">'
new_div = r'''<div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 min-h-[400px]" 
                id="question-{{ $index }}"
                x-show="currentTab === {{ $index }}"
                x-transition.opacity.duration.300ms>'''
html = re.sub(old_div, new_div, html)

# Replace the submission section
end_pattern = r'<div class="pt-8" x-data="\{ showConfirmModal: false \}" @open-confirm-modal\.window="showConfirmModal = true">[\s\S]*?<\/template>\s*<\/div>'
new_nav = '''
<!-- Navigasi Bawah -->
...
'''
html = re.sub(end_pattern, new_nav, html)
