<div x-data="examSecurity()">
    <div class="min-h-screen bg-gray-50/50 py-8 pb-20">
        <!-- Fullscreen Overlay -->
        <template x-teleport="body">
            <div x-show="!isFullscreen"
                ...
                <div class="bg-white p-8 rounded-2xl shadow-2xl max-w-md w-full text-center border border-gray-100">
                    ...
                    <p class="text-gray-600 mb-8 leading-relaxed">Anda keluar dari mode layar penuh atau memindahkan
                        tab.
                ...
                <div class="lg:w-3/4 w-full flex flex-col">
                    @foreach ($questions as $index => $question)
                        <div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-200 min-h-150 flex flex-col"
                            wire:key="question-box-{{ $question->id }}" id="question-{{ $index }}"
                            x-show="currentTab === {{ $index }}" x-transition:enter="transition ease-out duration-300"
                            ...
                            @if ($question->type === 'essay')
                                <div class="mt-4 grow">
                                    <textarea wire:model.blur="answers.{{ $question->id }}" rows="6"
                                    ...
                                    @foreach ($optionsList as $optionField)
                                        ...
                                        @if ($question->$optionField || $question->$imageField)
                                            <label class="flex items-start cursor-pointer group relative">
                                                <input type="radio" wire:model="answers.{{ $question->id }}" value="{{ $optionField }}"
                ...
                <div class="lg:w-1/4 w-full" x-data="{ showConfirmModal: false }"
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 sticky top-30">
                        ...
                        <div class="grid grid-cols-5 md:grid-cols-8 lg:grid-cols-4 xl:grid-cols-5 gap-2.5 mb-8">
                            @foreach ($questions as $index => $question)
                                <button type="button" wire:key="nav-dot-{{ $question->id }}"
                        ...
                        <div class="pt-6 border-t border-gray-100 flex flex-col gap-3">
                            ...
                            <div class="flex items-center gap-3 text-sm text-gray-600 mb-4">
                                <span class="w-5 h-3 rounded-sm border-b-[3px] border-b-amber-500 bg-gray-100"></span>
                                Soal
                                Essai
                            </div>
        ...
        <script>
            Alpine.data('examSecurity', () => ({
                ...
                init() {
                    ...
                    // Auto-save progres setiap 30 detik (menggantikan sync per klik)
                    setInterval(() => {
                        $wire.syncProgress();
                    }, 30000);

                    // Keyboard shortcut navigation (Left/Right arrows) — tanpa server call
    ...
    </div> <!-- End of Parent Root Element -->
</div>